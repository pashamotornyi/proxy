...
# Жёсткий приоритет портов: сначала 22, потом 2222
SSH_PORTS = [22, 2222]
CONNECT_TIMEOUT = 10
LOGIN_TIMEOUT = 45
...
BUILD_TAG = "bot-ports-22-first-2025-09-29-14-59"
...
# ===== Подключение: 22 -> 2222, с диагностикой =====
async def connect_resilient(host: str, username: str, password: str):
    reasons = []
    for port in SSH_PORTS:  # [22, 2222]
        try:
            ...
            # собираем краткую причину, но продолжаем к следующему порту
            reasons.append(f"порт {port}: {e}")
            await asyncio.sleep(0.5)
    # если сюда дошли — оба порта не дали коннект
    raise RuntimeError("SSH не доступен: " + "; ".join(reasons))

# ===== Исполнение установки (с подавлением обрыва после финала) =====
...
@bot.event
async def on_ready():
    print(f"Starting ProxySetup 22-first, {BUILD_TAG}")
