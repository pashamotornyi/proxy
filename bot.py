...
# SSH параметры
SSH_PORTS = [22]  # работаем только по 22
CONNECT_TIMEOUT = 20
LOGIN_TIMEOUT = 90
KEEPALIVE_INTERVAL = 15

# Расширенный, но безопасный набор алгоритмов для широкой совместимости
...
BUILD_TAG = "bot-pty-finish-suppress-2025-09-30-14-16"

# Логирование клиента (можно поднять до DEBUG при отладке)
...
async def connect_resilient(host: str, username: str, password: str, interaction: discord.Interaction):
    ...
    for port in SSH_PORTS:
        # экспоненциальные ретраи с небольшим джиттером
        for attempt in range(3):
            try:
                ...
                try:
                    cipher = conn.get_extra_info('cipher')
                    kex = conn.get_extra_info('kex')
                    hostkeys = conn.get_server_host_key_algs()
                ...
                last_exc = e
                try_name = f"{host}:{port} try {attempt+1}"
                await interaction.followup.send(
                    f"[ssh] connect failed {try_name}: {type(e).__name__}: {e}",
                    ephemeral=True
                )
                # пауза с ростом
                ...
                async def drain_stream(stream, is_err=False):
                    async for raw in stream:
                        ...
                        if line.startswith("==="):
                            ...
                            # опционально можно ограничить шум обычного stdout
                            pass
