...
ALLOWED_USERS = {int(x) for x in os.environ.get("ALLOWED_USERS", "").split(",") if x.strip().isdigit()}  # белый список [3]
ALLOW_ALL = os.environ.get("ALLOW_ALL", "") == "1"  # разрешить всем (отладка) [3]
QUIET = os.environ.get("QUIET", "") == "1"          # тихий режим статусов [3]

SSH_KNOWN_HOSTS = None  # в проде задайте known_hosts для проверки ключей сервера [6]
...
"""  # задаёт UTF‑8 без apt, безопасно и тихо [2]
...
class StartView(discord.ui.View):
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not user_allowed_ctx(interaction):
            ...
            return
        await interaction.response.defer(ephemeral=True)  # подтверждаем интеракцию как эпемерную [7]
        try:
            dm = await interaction.user.create_dm()
            await dm.send("Выберите тип сервера:", view=RoleView())  # переходим в DM [3]
            await interaction.followup.send("Открыл личные сообщения.", ephemeral=True)  # уведомление [7]
        except discord.Forbidden:
            await interaction.followup.send("Не удалось написать в личные сообщения (закрыт DM).", ephemeral=True)  # обработка закрытого DM [7]
...
class IntermediateModal(discord.ui.Modal, title="Промежуточный сервер"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not valid_ip(str(self.forward_ip)):
            return await interaction.followup.send("Некорректный forward_ip.", ephemeral=True)  # валидация IP [3]
        params = dict(
        ...
        )  # параметры подключения [3]
        await run_remote_setup(interaction, mode="intermediate", params=params)  # запуск мастера [3]
...
class FinalModal(discord.ui.Modal, title="Финальный сервер"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await run_remote_setup(interaction, mode="final", params=params)  # запуск мастера [3]
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, allow_redirects=True) as resp:
            ...
            if resp.status != 200 or not data:
                raise RuntimeError(f"Download failed: HTTP {resp.status}")  # защита от плохого ответа [5]
    text = data.decode("utf-8", "replace").replace("\r\n", "\n").replace("\r", "\n")
    if not text.startswith("#!"):
        raise RuntimeError("Downloaded content is not a script (no shebang)")  # защита от HTML вместо скрипта [5]
...
async def sftp_upload(conn: asyncssh.SSHClientConnection, data: bytes, remote_path: str) -> None:
    async with conn.start_sftp_client() as sftp:
        async with sftp.open(remote_path, "wb") as f:
            await f.write(data)  # бинарная запись по SFTP [1]
...
async def run_silent(conn: asyncssh.SSHClientConnection, cmd: str, use_bash: bool = False):
    exec_cmd = cmd if not use_bash else f'/bin/bash -lc {sh_esc(cmd)}'  # единый путь запуска [1]
    result = await conn.run(exec_cmd, check=False)  # не выбрасываем исключение, возвращаем код [1]
    return result.exit_status, (result.stdout or ""), (result.stderr or "")  # rc/stdout/stderr [1]

async def run_step(send, title: str, coro):
    await send(f"— {title} —")  # заголовок шага [3]
    try:
    ...
    async with conn.create_process(exec_cmd) as proc:
        async for line in proc.stdout:
            if ("=== [" in line) or ("Ок" in line) or ("Ошибка" in line):
                await send(line.strip())  # только маркеры [3]
        rc = await proc.wait()
    await send("Ок" if rc == 0 else f"Ошибка (код {rc})")  # финальный статус по rc [1]
...
async def run_remote_setup(interaction: discord.Interaction, mode: str, params: dict):
    ...
    try:
        async with asyncssh.connect(**conn_kwargs) as conn:
            ...
            try:
                content = await download_script(SCRIPT_URL)  # загрузка на стороне бота [5]
                await sftp_upload(conn, content, "setup_reboot.sh")  # отправка по SFTP [1]
                await send("Ок")
            except Exception as e:
            ...
            # 3) Запуск установки
            if mode == "final":
                run_cmd = f'./setup_reboot.sh --final --password {sh_esc(params["ss_password"])}'  # команда финального [3]
            else:
                run_cmd = f'./setup_reboot.sh --forward-ip {sh_esc(params["forward_ip"])} --password {sh_esc(params["ss_password"])}'  # команда промежуточного [3]
