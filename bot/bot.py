...
def sh_esc(s: str) -> str:
    # Важно: экранировать двойные кавычки как \", а в Python для этого нужен двойной бэкслеш \\
    return '"' + s.replace('"', '\\"') + '"'  # корректное экранирование для /bin/bash -lc [1]
...
"""  # задаёт UTF‑8 на сессию и старается записать системно, без apt и жёстких падений [5]

# ================= UI: кнопки и модалки =================
class StartView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)  # без таймаута [4]
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not user_allowed_ctx(interaction):
            await interaction.response.send_message("Недостаточно прав для запуска мастера.", ephemeral=True)  # приватный отказ [4]
            return
        await interaction.response.defer(ephemeral=True)  # корректный эпемерный defer перед followup [6]
        try:
            dm = await interaction.user.create_dm()
            await dm.send("Выберите тип сервера:", view=RoleView())  # переход в DM [4]
...
class RoleView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=600)  # 10 минут [4]
    ...
    async def inter(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(IntermediateModal(title="Промежуточный сервер"))  # модалка [4]
    ...
    async def final(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(FinalModal(title="Финальный сервер"))  # модалка [4]

class IntermediateModal(discord.ui.Modal, title="Промежуточный сервер"):
    host = discord.ui.TextInput(label="Хост (IP/домен)", placeholder="1.2.3.4", required=True)  # [4]
    ssh_pass = discord.ui.TextInput(label="SSH пароль", required=True)  # [4]
    forward_ip = discord.ui.TextInput(label="IP следующего сервера", placeholder="5.6.7.8", required=True)  # [4]
    ss_password = discord.ui.TextInput(label="Пароль Shadowsocks", required=True, min_length=6, max_length=64)  # [4]

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True, thinking=True)  # индикатор [4]
        if not valid_ip(str(self.forward_ip)):
            return await interaction.followup.send("Некорректный forward_ip.", ephemeral=True)  # валидация [4]
        params = dict(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, allow_redirects=True) as resp:
            ...
            if resp.status != 200 or not data:
                raise RuntimeError(f"Download failed: HTTP {resp.status}")  # защита [7]
    text = data.decode("utf-8", "replace").replace("\r\n", "\n").replace("\r", "\n")
    if not text.startswith("#!"):
        raise RuntimeError("Downloaded content is not a script (no shebang)")  # валидация [7]
    return text.encode("utf-8")  # нормализованный UTF‑8 [7]
...
async def run_silent(conn: asyncssh.SSHClientConnection, cmd: str, use_bash: bool = False):
    ...
    result = await conn.run(exec_cmd, check=False)  # без поднятия исключений [1]
    return result.exit_status, (result.stdout or ""), (result.stderr or "")  # rc/out/err [1]

async def run_step(send, title: str, coro):
    await send(f"— {title} —")  # заголовок секции
    try:
        rc, out, err = await coro
        if rc == 0:
            await send("Ок")  # успех по коду возврата [2]
            return True
...
async def run_and_stream(conn: asyncssh.SSHClientConnection, cmd: str, send, title: str = "", use_bash: bool = False) -> int:
    # Упростили: транслируем только маркеры прогресса, НЕ ищем “Ошибка” в тексте, финальный статус — строго по rc
    exec_cmd = cmd if not use_bash else f'/bin/bash -lc {sh_esc(cmd)}'  # запуск [1]
    if title:
        await send(f"— {title} —")  # заголовок
    async with conn.create_process(exec_cmd) as proc:
        async for line in proc.stdout:
            if "=== [" in line:
                await send(line.strip())  # транслируем только маркеры прогресса
        rc = await proc.wait()
    ...
    try:
        async with asyncssh.connect(**conn_kwargs) as conn:  # SSH-сессия [1]
            ...
            if QUIET:
                ...
                if not ok:
                    return  # выходим только при rc!=0
            else:
            ...
            try:
                content = await download_script(SCRIPT_URL)  # скачиваем на боте [7]
        ...
        await interaction.followup.send(f"Ошибка SSH/выполнения: {e}", ephemeral=True)  # общий перехват [6]

# ================= Инициализация и публикация кнопки =================
@bot.event
async def on_ready():
    try:
        await bot.tree.sync()  # синхронизация app-команд [4]
    except Exception as e:
