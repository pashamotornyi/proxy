...
SSH_KNOWN_HOSTS = None  # при проде используйте проверку host key known_hosts [3]
...
def user_allowed_ctx(interaction: discord.Interaction) -> bool:
    ...
    if interaction.guild:
        member = interaction.guild.get_member(interaction.user.id) or interaction.user  # членство в гильдии [1]
        ...
        return isinstance(member, discord.Member) and member.guild_permissions.administrator  # по умолчанию — администраторы [1]
    if ALLOWED_CHANNEL_ID:
        ch = interaction.client.get_channel(ALLOWED_CHANNEL_ID)  # канал из настроек [1]
        guild = getattr(ch, "guild", None)  # гильдия канала [1]
        if guild:
            member = guild.get_member(interaction.user.id)  # поиск участника [1]
            if member:
                if ALLOWED_ROLE and any(r.name == ALLOWED_ROLE for r in member.roles):
                    return True  # проверка роли [1]
                return member.guild_permissions.administrator  # проверка администратора [1]
    return False  # запрет по умолчанию [1]
...
locale | sed -n '1,8p'
"""  # генерирует en_US.UTF‑8 и активирует по умолчанию, выводит первые строки locale для проверки [4][6]
...
class StartView(discord.ui.View):
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await dm.send("Выберите тип сервера:", view=RoleView())  # переход в мастер [1]
...
class IntermediateModal(discord.ui.Modal, title="Промежуточный сервер"):
    host = discord.ui.TextInput(label="Хост (IP/домен)", placeholder="1.2.3.4", required=True)  # поле 1 [7]
    ssh_pass = discord.ui.TextInput(label="SSH пароль", required=True)  # поле 2 [7]
    forward_ip = discord.ui.TextInput(label="IP следующего сервера", placeholder="5.6.7.8", required=True)  # поле 3 [7]
    ss_password = discord.ui.TextInput(label="Пароль Shadowsocks", required=True, min_length=6, max_length=64)  # поле 4 [7]

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True, thinking=True)  # индикатор [1]
        if not valid_ip(str(self.forward_ip)):
            return await interaction.followup.send("Некорректный forward_ip.", ephemeral=True)  # валидация IP [5]
        params = dict(
            host=str(self.host),
            user="root",
            port=22,
...
class FinalModal(discord.ui.Modal, title="Финальный сервер"):
    host = discord.ui.TextInput(label="Хост (IP/домен)", placeholder="1.2.3.4", required=True)  # поле 1 [7]
    ssh_pass = discord.ui.TextInput(label="SSH пароль", required=True)  # поле 2 [7]
    ss_password = discord.ui.TextInput(label="Пароль Shadowsocks", required=True, min_length=6, max_length=64)  # поле 3 [7]
...
async def run_remote_setup(interaction: discord.Interaction, mode: str, params: dict):
    ...
    try:
        async with asyncssh.connect(**conn_kwargs) as conn:  # сессия SSH [3]
            ...
            # 2) Загрузка и проверка скрипта через wget с редиректами и валидацией шебанга
            fetch_cmd = (
                f'wget -qO- -L "{SCRIPT_URL}" 2>/dev/null '
                f'| sed \'s/\\r$//\' | tee setup_reboot.sh > /dev/null && '
                f'[ -s setup_reboot.sh ] && head -c 2 setup_reboot.sh | grep -q "^#!" && chmod +x setup_reboot.sh || '
                f'{{ echo "Invalid download (empty or not a script)"; rm -f setup_reboot.sh; exit 1; }}'
            )  # -L следует редиректам; проверяем, что файл не пуст и начинается с #! [2][1]
