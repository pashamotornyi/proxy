...
SCRIPT_URL = os.environ["SCRIPT_URL"]                                  # RAW URL на setup_reboot.sh [8]
...
SSH_KNOWN_HOSTS = None  # для продакшена задайте known_hosts/проверку host key [5]
...
class StartView(discord.ui.View):
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not user_allowed_ctx(interaction):
            ...
            return
        await interaction.response.defer(ephemeral=True)  # корректный defer перед followup [8]
        try:
            ...
            await interaction.followup.send("Не удалось написать в личные сообщения (закрыт DM).", ephemeral=True)  # обработка отказа [8]
...
class IntermediateModal(discord.ui.Modal):
    def __init__(self, title: str):
        super().__init__(title=title)  # явная инициализация [8]
        ...
        for comp in (self.host, self.ssh_pass, self.forward_ip, self.ss_password):
            self.add_item(comp)  # стабильное добавление полей [8]

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True, thinking=True)  # индикатор [8]
        if not valid_ip(str(self.forward_ip.value)):
            return await interaction.followup.send("Некорректный forward_ip.", ephemeral=True)  # валидация [8]
        params = dict(
        ...
        await run_remote_setup(interaction, mode="intermediate", params=params)  # запуск [8]
...
class FinalModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await run_remote_setup(interaction, mode="final", params=params)  # запуск [8]
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, allow_redirects=True) as resp:
            ...
            if resp.status != 200 or not data:
                raise RuntimeError(f"Download failed: HTTP {resp.status}")  # защита [8]
    text = data.decode("utf-8", "replace").replace("\r\n", "\n").replace("\r", "\n")
    if not text.startswith("#!"):
        raise RuntimeError("Downloaded content is not a script (no shebang)")  # валидация [8]
    return text.encode("utf-8")  # нормализованный UTF‑8 [8]

async def sftp_upload(conn: asyncssh.SSHClientConnection, data: bytes, remote_path: str) -> None:
    async with conn.start_sftp_client() as sftp:
        async with sftp.open(remote_path, "wb") as f:
            await f.write(data)  # запись bytes по SFTP [5]
    await conn.run(f"chmod +x {sh_esc(remote_path)}", check=True)  # делаем исполняемым [5]
...
async def run_and_stream(conn: asyncssh.SSHClientConnection, cmd: str, send, title: str = "") -> int:
    if title:
        await send(f"— {title} —")  # заголовок [8]
    async with conn.create_process(cmd) as proc:
        async for line in proc.stdout:
            if "=== [" in line:
                await send(line.strip())  # транслируем только маркеры прогресса [23]
        rc = await proc.wait()
    await send("Ок" if rc == 0 else f"Ошибка (код {rc})")  # финальный статус по rc [24]
    return rc  # rc наружу [24]

async def run_step(send, title: str, coro):
    await send(f"— {title} —")  # заголовок секции [8]
    try:
        rc, out, err = await coro
        if rc == 0:
            await send("Ок")  # успех [24]
            return True
        tail_src = (err or out or "").strip().splitlines()[-3:]
        suffix = (": " + " | ".join(tail_src)) if tail_src else ""
        await send(f"Ошибка (код {rc}){suffix}")  # краткий контекст при ошибке [23]
        return False
    except Exception as e:
        await send(f"Ошибка: {e}")  # исключение шага [23]
        return False
...
async def run_remote_setup(interaction: discord.Interaction, mode: str, params: dict):
    ...
    try:
        async with asyncssh.connect(**conn_kwargs) as conn:  # SSH‑сессия [1]
            ...
            try:
                content = await download_script(SCRIPT_URL)  # скачиваем на боте [8]
                await sftp_upload(conn, content, "setup_reboot.sh")  # отправляем по SFTP [5]
                await send("Ок")
            except Exception as e:
            ...
            # 3) Повторное меню
            await interaction.followup.send("Выберите тип сервера:", view=RoleView(), ephemeral=True)  # новое меню [8]
