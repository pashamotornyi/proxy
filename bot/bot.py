...
TOKEN = os.environ["DISCORD_BOT_TOKEN"]            # токен бота [8]
SCRIPT_URL = os.environ["SCRIPT_URL"]              # RAW URL на setup_reboot.sh [10]
ALLOWED_CHANNEL_ID = int(os.environ.get("ALLOWED_CHANNEL_ID", "0"))  # канал для кнопки [8]
ALLOWED_ROLE = os.environ.get("ALLOWED_ROLE", "")                    # имя роли (опц.) [8]
ALLOWED_USERS = {int(x) for x in os.environ.get("ALLOWED_USERS", "").split(",") if x.strip().isdigit()}  # белый список ID [8]
ALLOW_ALL = os.environ.get("ALLOW_ALL", "") == "1"  # разрешить всем (отладка) [8]
QUIET = os.environ.get("QUIET", "") == "1"          # тихий режим статусов [8]

SSH_KNOWN_HOSTS = None  # для продакшена настройте known_hosts/проверку host key [12]

# ================= Discord клиент =================
intents = discord.Intents.default()  # для UI достаточно default [8]
bot = commands.Bot(command_prefix="!", intents=intents)  # основной клиент [8]
...
def valid_ip(v: str) -> bool:
    try:
        ...
        return False  # простая валидация IPv4/IPv6 [8]

def sh_esc(s: str) -> str:
    return '"' + s.replace('"', '\\"') + '"'  # безопасная цитата для bash -lc [8]

def user_allowed_ctx(interaction: discord.Interaction) -> bool:
    if ALLOW_ALL:
        return True  # принудительный допуск [8]
    if ALLOWED_USERS and interaction.user.id in ALLOWED_USERS:
        return True  # белый список [8]
    if interaction.guild:
        ...
        if ALLOWED_ROLE and isinstance(member, discord.Member) and any(r.name == ALLOWED_ROLE for r in member.roles):
            return True  # роль [8]
        return isinstance(member, discord.Member) and member.guild_permissions.administrator  # админ [8]
        ...
        if guild:
            ...
            if member:
                if ALLOWED_ROLE and any(r.name == ALLOWED_ROLE for r in member.roles):
                    return True  # роль [8]
                return member.guild_permissions.administrator  # админ [8]
    return False  # запрет [8]
...
locale | sed -n '1,8p'
"""  # включает en_US.UTF‑8; при QUIET=0 видно первые строки locale [9]

# ================= UI: кнопки и модалки =================
class StartView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)  # без таймаута [8]
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not user_allowed_ctx(interaction):
            await interaction.response.send_message("Недостаточно прав для запуска мастера.", ephemeral=True)  # приватный отказ [8]
            return
        await interaction.response.defer(ephemeral=True)  # подтверждение [8]
        try:
            dm = await interaction.user.create_dm()
            await dm.send("Выберите тип сервера:", view=RoleView())  # переход в DM [8]
...
class RoleView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=600)  # 10 минут [8]
    ...
    async def inter(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(IntermediateModal(title="Промежуточный сервер"))  # модалка [8]
    ...
    async def final(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(FinalModal(title="Финальный сервер"))  # модалка [8]

class IntermediateModal(discord.ui.Modal, title="Промежуточный сервер"):
    host = discord.ui.TextInput(label="Хост (IP/домен)", placeholder="1.2.3.4", required=True)  # [8]
    ssh_pass = discord.ui.TextInput(label="SSH пароль", required=True)  # [8]
    forward_ip = discord.ui.TextInput(label="IP следующего сервера", placeholder="5.6.7.8", required=True)  # [8]
    ss_password = discord.ui.TextInput(label="Пароль Shadowsocks", required=True, min_length=6, max_length=64)  # [8]

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True, thinking=True)  # [8]
        if not valid_ip(str(self.forward_ip)):
            return await interaction.followup.send("Некорректный forward_ip.", ephemeral=True)  # [8]
        params = dict(
        ...
        await run_remote_setup(interaction, mode="intermediate", params=params)  # [8]
...
class FinalModal(discord.ui.Modal, title="Финальный сервер"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await run_remote_setup(interaction, mode="final", params=params)  # [8]
...
async def sftp_upload(conn: asyncssh.SSHClientConnection, data: bytes, remote_path: str) -> None:
    async with conn.start_sftp_client() as sftp:  # [9]
        async with sftp.open(remote_path, "wb") as f:
            await f.write(data)  # [9]
    await conn.run(f"chmod +x {sh_esc(remote_path)}", check=True)  # [9]
...
async def run_silent(conn: asyncssh.SSHClientConnection, cmd: str, use_bash: bool = False):
    exec_cmd = cmd if not use_bash else f'/bin/bash -lc {sh_esc(cmd)}'  # [9]
    result = await conn.run(exec_cmd, check=False)  # [9]
    return result.exit_status, (result.stdout or ""), (result.stderr or "")  # [9]

async def run_step(send, title: str, coro):
    await send(f"— {title} —")  # [8]
    try:
        rc, out, err = await coro
        if rc == 0:
            await send("Ок")  # успех по коду возврата [9]
            return True
        else:
            tail_src = (err or out or "").strip().splitlines()[-3:]
            suffix = (": " + " | ".join(tail_src)) if tail_src else ""
            await send(f"Ошибка (код {rc}){suffix}")  # краткая ошибка [9]
            return False
    except Exception as e:
        await send(f"Ошибка: {e}")  # [9]
        return False
...
async def run_and_stream(conn: asyncssh.SSHClientConnection, cmd: str, send, title: str = "", use_bash: bool = False) -> int:
    ...
    if title:
        await send(f"— {title} —")  # [8]
    async with conn.create_process(exec_cmd) as proc:  # [9]
        async for line in proc.stdout:
            if ("=== [" in line) or ("Ок" in line) or ("Ошибка" in line):
                await send(line.strip())  # только маркеры [8]
        rc = await proc.wait()
    await send("Ок" if rc == 0 else f"Ошибка (код {rc})")  # успех по коду возврата [9]
    return rc  # [9]
...
async def run_remote_setup(interaction: discord.Interaction, mode: str, params: dict):
    async def send(text: str):
        ...
        if chunk.strip():
            await interaction.followup.send(chunk, ephemeral=True)  # followup для подтверждённой интеракции [11]

    await send("Подключение по SSH и проверка локали (UTF‑8)…")  # [9]
    conn_kwargs = dict(
    ...
    try:
        async with asyncssh.connect(**conn_kwargs) as conn:  # [9]
            # 1) Локаль
            if QUIET:
            ...
            try:
                ...
                await sftp_upload(conn, content, "setup_reboot.sh")  # [9]
                await send("Ок")
            except Exception as e:
            ...
            # 3) Запуск установки
            if mode == "final":
                run_cmd = f'./setup_reboot.sh --final --password {sh_esc(params["ss_password"])}'  # [8]
            else:
                run_cmd = f'./setup_reboot.sh --forward-ip {sh_esc(params["forward_ip"])} --password {sh_esc(params["ss_password"])}'  # [8]
            ...
            # 4) После успеха — сразу новое меню для следующего сервера
            await interaction.followup.send("Выберите тип сервера:", view=RoleView(), ephemeral=True)  # [8]
    except Exception as e:
        await interaction.followup.send(f"Ошибка SSH/выполнения: {e}", ephemeral=True)  # [11]

# ================= Инициализация и публикация кнопки =================
@bot.event
async def on_ready():
    try:
        await bot.tree.sync()  # синхронизация app-команд [8]
    except Exception as e:
        ...
        if ch:
            try:
                await ch.send(
                    ...
                    view=StartView()
                )  # публичная стартовая кнопка [8]
            except Exception as e:
                print("Failed to send start message:", e)

bot.run(TOKEN)  # запуск клиента [8]
