...
if [[ -z "$FORWARD_IP" || -z "$PASSWORD" ]]; then
  echo "Использование: $0 <FORWARD_IP> <PASSWORD> [--final] [--add-hosts] [--disable-ufw]"
  exit 1
fi
...
echo "[2/10] Установка базовых пакетов и утилит..."
...
echo "[3/10] Установка Docker и Compose..."
...
echo "[4/10] Установка системных мониторов..."
...
if $DISABLE_UFW; then
  echo "[5/10] Отключение ufw..."
  sudo ufw disable
fi

if ! $IS_FINAL; then
  echo "[6/10] Настройка промежуточного прокси сервера через Docker (Glider)..."
...
if $IS_FINAL; then
  echo "[7/10] Настройка конечного сервера (Shadowsocks-libev)..."
...
WantedBy=multi-user.target" | sudo tee /etc/systemd/system/shadowsocks-libev-server@.service > /dev/null
  ...
  if $ADD_HOSTS; then
    echo "[8/10] Поиск IP Binance с минимальным пингом..."
    BINANCE_IPS=(
    ...
    for ip in "${BINANCE_IPS[@]}"; do
      ...
      count=$((count + 1))
      echo -ne "Проверено $count/$total IP
"
    done
...
echo "✅ Установка завершена. Перезагрузите сервер при необходимости: sudo reboot"
