...
# Если файл состояния существует — продолжаем установку
if [[ -f "$STATE_FILE" ]]; then
  echo "♻️ Обнаружено состояние. Продолжаем установку после перезагрузки..."
  source "$STATE_FILE"
  START_STEP=2
else
  FORWARD_IP="$1"
...
if [[ $START_STEP -eq 1 ]]; then
  ...
  echo "💾 Сохраняем состояние для продолжения после перезагрузки..."
  cat > "$STATE_FILE" <<EOF
...
EOF
  ...
  echo "⚙️ Настраиваем автозапуск после перезагрузки..."
  cat > /etc/systemd/system/$SERVICE_NAME.service <<EOF
[Unit]
Description=Continue Proxy Setup After Reboot
...
if [[ $START_STEP -eq 2 ]]; then
  echo "🚀 Продолжаем установку (шаг 2)..."

  echo "[2/10] Installing essential packages and utilities..."
  sudo apt install -y mc nano net-tools htop zip httpie \
...
https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable" | \
  ...
  curl -L https://github.com/bcicen/ctop/releases/download/v0.7.7/ctop-0.7.7-linux-amd64 \
  ...
  api:
    ...
    logging:
      ...
      options:
        max-size: '800k'
        max-file: '10'
    command: -verbose -listen ss://AEAD_AES_256_GCM:${PASSWORD}@api:8388 \
...
WantedBy=multi-user.target" | sudo tee /etc/systemd/system/shadowsocks-libev-server@.service > /dev/null
  ...
  echo "🧹 Убираем временные файлы и автозапуск..."
